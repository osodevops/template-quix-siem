...
func findMatchInternal(jsonString string) string {
	...
	fmt.Println("Golang - Start matching against all rules...")
	for _, ruleEvaluator := range rulesMap {
		fmt.Print(" ", ruleEvaluator.ID)
		result, err := ruleEvaluator.Matches(context.Background(), resultMap)

		if err != nil {
			fmt.Println("\nGolang - Error occurred matching rule to data for Id: ", ruleEvaluator.ID, "For data: ", jsonString)
			continue
		}

		if result.Match {
			matchingIds = append(matchingIds, ruleEvaluator.ID)
			fmt.Println("\nGolang - Matching found for Id: ", ruleEvaluator.ID, "For data: ", jsonString)
		}
	}

	fmt.Println("\nGolang - Completed scan")
